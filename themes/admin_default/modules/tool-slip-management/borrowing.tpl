...
<div class="card shadow-sm mb-4">
    ...
    <h5 class="card-title mb-0"><i class="fas fa-plus-circle mr-2"></i>{LANG.create_borrowing_slip}</h5>
</div>
<div class="card-body">
...
<div class="row">
    <div class="col-md-6">
        <div class="form-group">
                <label class="font-weight-bold"><i class="fas fa-user mr-1"></i>{LANG.student}</label>
            ...
            <div class="col-md-3">
                <div class="form-group">
                    <label class="font-weight-bold"><i class="fas fa-calendar-alt mr-1"></i>{LANG.borrow_date}</label>
                    <input type="date" class="form-control" name="borrow_date" value="{DATA.borrow_date}" required>
                </div>
            </div>
            <div class="col-md-3">
            <div class="form-group">
                <label class="font-weight-bold"><i class="fas fa-clock mr-1"></i>{LANG.due_date}</label>
                    <input type="date" class="form-control" name="due_date" value="{DATA.due_date}" required>
                </div>
        </div>
    </div>
        <div class="form-group">
        <label class="font-weight-bold"><i class="fas fa-tools mr-1"></i>{LANG.tools}</label>
        <div class="input-group mb-2">
            ...
            <div class="form-group">
                <label class="font-weight-bold"><i class="fas fa-sticky-note mr-1"></i>{LANG.note}</label>
            ...
            <div class="form-group text-right">
                <button type="submit" name="submit" class="btn btn-success btn-lg"><i class="fas fa-save mr-1"></i>{GLANG.submit}</button>
                <a href="{NV_BASE_ADMINURL}index.php?{NV_LANG_VARIABLE}={NV_LANG_DATA}&{NV_NAME_VARIABLE}={MODULE_NAME}&{NV_OP_VARIABLE}=borrowing" class="btn btn-outline-secondary btn-lg ml-2"><i class="fas fa-times mr-1"></i>{GLANG.cancel}</a>
            </div>
        </form>
    </div>
</div>
...
<h5 class="card-title mb-0"><i class="fas fa-list mr-2"></i>{LANG.borrowing_management}</h5>
<button id="btn-add-slip" class="btn btn-primary"><i class="fas fa-plus mr-1"></i>{LANG.create_borrowing_slip}</button>
</div>
...
<div class="input-group input-group-sm">
        <div class="input-group-prepend">
            <span class="input-group-text"><i class="fas fa-search"></i></span>
                ...
                <input type="text" id="search-input" class="form-control" placeholder="Tìm kiếm theo mã sinh viên, tên..." />
                </div>
            </div>
            <div class="col-md-4">
...
<div class="card shadow-sm mb-4 view-slip">
    ...
    <h5 class="card-title mb-0"><i class="fas fa-eye mr-2"></i>{LANG.view_borrowing_slip} #{SLIP.id}</h5>
</div>
<div class="card-body">
<div class="row">
    <div class="col-md-6">
        <div class="mb-3">
            <label class="font-weight-bold text-muted"><i class="fas fa-user mr-1"></i>{LANG.student}</label>
            <p class="mb-0">{SLIP.full_name} <small class="text-muted">({SLIP.student_code})</small><br><small class="text-muted">SĐT: {SLIP.phone_number}</small></p>
        </div>
    </div>
    <div class="col-md-6">
    <div class="mb-3">
        <label class="font-weight-bold text-muted"><i class="fas fa-info-circle mr-1"></i>{LANG.status}</label>
        <p class="mb-0">
                <span class="label label-{SLIP.status_class}">{SLIP.status_text}</span>
                    </p>
                </div>
        ...
        <div class="row">
            <div class="col-md-4">
                <div class="mb-3">
                    <label class="font-weight-bold text-muted"><i class="fas fa-calendar-alt mr-1"></i>{LANG.borrow_date}</label>
                    <p class="mb-0">{SLIP.borrow_date}</p>
                </div>
            </div>
            <div class="col-md-4">
                <div class="mb-3">
                    <label class="font-weight-bold text-muted"><i class="fas fa-clock mr-1"></i>{LANG.due_date}</label>
                    <p class="mb-0 {SLIP.status == 2 ? 'text-danger' : ''}">{SLIP.due_date}</p>
                </div>
            </div>
            <div class="col-md-4">
                <div class="mb-3">
                    <label class="font-weight-bold text-muted"><i class="fas fa-undo mr-1"></i>{LANG.return_date}</label>
                    <p class="mb-0">{SLIP.return_date}</p>
                </div>
            </div>
        </div>
        <!-- IF SLIP.notes -->
        <div class="mb-3">
        <label class="font-weight-bold text-muted"><i class="fas fa-sticky-note mr-1"></i>{LANG.note}</label>
        <p class="mb-0">{SLIP.notes}</p>
        </div>
        <!-- ENDIF -->
        <div class="mb-3">
            <label class="font-weight-bold text-muted"><i class="fas fa-tools mr-1"></i>{LANG.tools}</label>
    ...
    <div class="card-footer text-right">
        <a href="{NV_BASE_ADMINURL}index.php?{NV_LANG_VARIABLE}={NV_LANG_DATA}&{NV_NAME_VARIABLE}={MODULE_NAME}&{NV_OP_VARIABLE}=borrowing" class="btn btn-outline-secondary"><i class="fas fa-arrow-left mr-1"></i>{GLANG.back}</a>
    </div>
</div>
<!-- END: view -->
